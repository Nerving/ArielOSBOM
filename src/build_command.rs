...
impl ArielOsBuildCommand {
    ...
    pub fn from_compile_commands_json(project_path: &Path) -> ArielOsBuildCommand {
        ArielOsBuildCommand::parse_build_command(CompileCommandsJson::from_compile_commdands_json(project_path))
    }
            ...
            Ok(output) => {
                if output.status.success() {
                    ...
                    match env::var("TESTING") {
                        Ok(value) if value == "1" => panic!("SBOM generation failed for builder {}:\n\t{}", context.builder, String::from_utf8_lossy(&output.stderr)),
                        _ => println!("failed to generate build files for builder {}:\n\t{}", context.builder, String::from_utf8_lossy(&output.stderr))
                    };
                }
            },
            Err(_) => println!("error executing laze for builder {}: {:?}", context.builder, laze_output.err())
        };
