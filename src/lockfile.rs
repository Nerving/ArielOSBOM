...
pub fn generate_checksum_map(
    ...
    tree_set: HashSet<String>,
) -> HashMap<(String, String), Checksum> {
            ...
            .map(|lock_package| {
                (
                    (
                        ...
                        lock_package.version.to_string(),
                    ),
...
fn enrich_lockfile(
    ...
    let main_lockfile_package_set: HashSet<String> = main_lockfile
        .packages
        .iter()
    ...
    let missing_checksum_list: HashSet<&String> = HashSet::from_iter(
        tree_set
            .iter()
            .filter(|entry| !main_lockfile_package_set.contains(*entry)),
    );
...
fn extract_missing_checksums(
    checklist: HashSet<&String>,
    import_lockdata: Vec<LockPackage>,
) -> Vec<LockPackage> {
    import_lockdata
        .into_iter()
        .filter(|package| {
            checklist.contains(&format!("{} v{}", package.name.as_str(), package.version))
        })
